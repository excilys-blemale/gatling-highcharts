...
class RequestDetailsIndicatorTemplate(expand: Boolean) extends Template {

	def getJSContent = PageTemplate.TEMPLATE_ENGINE.layout(REQUEST_DETAILS_INDICATORS_JS_TEMPLATE_URL,
		Map("chartTitle" -> "Indicators", "expand" -> expand))

	def getHTMLContent = PageTemplate.TEMPLATE_ENGINE.layout(REQUEST_DETAILS_INDICATORS_HTML_TEMPLATE_URL, Map("expand" -> expand))
}
